...
class TrxShipment extends CI_Controller {
	public function index() {
		if($this->session->userdata('id_role') == "1" || $this->session->userdata('id_role') == "4" || $this->session->userdata('id_role') == "6") {
			...
			$this->load->view('menu');
			$this->load->view('trxshipment/shipment_tabel.php');
	...
	public function save() {
		if($this->session->userdata('id_role') == "1" || $this->session->userdata('id_role') == "4" || $this->session->userdata('id_role') == "6") {
			if($this->session->userdata("id_role")==6){
				$required = array('id_request','id_shipping','tanggal_shipping','tipe','id_status_kirim','id_jenis_transaksi');
				$error = false;
				...
				$required = array('id_request','id_shipping','tanggal_shipping','tipe','id_status_kirim','id_shipping_point','id_detail_request','id_jenis_transaksi');
				$error = false;
				foreach($required as $field) {
					if(empty($_POST[$field])) {
						$error = true;
						...
						if($this->input->post('id_status_kirim')==4){							
							if($this->input->post('id_jenis_transaksi')==1 || $this->input->post('id_jenis_transaksi')==2){
								...
								$qShp =  $this->db->query("SELECT shipping.* FROM shipping JOIN detail_request ON shipping.id_detail_request=detail_request.id_detail_request 
														WHERE shipping.id_request= '$id_request' AND id_jenis_transaksi='$id_jenis_transaksi'");
								...
								$qDt =  $this->db->query("SELECT * FROM detail_request WHERE id_request='$id_request' AND id_jenis_transaksi='$id_jenis_transaksi'");
	...
	public function get_row($id) {
		...
		$this->load->view('trxshipment/detail.php',$d);
		$this->load->view('bottom');
	}
