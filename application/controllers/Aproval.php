...
class Aproval extends CI_Controller {
	...
	public function get_aproval() {
		...
		$no = 1;	
		$get = $this->db->query("SELECT * FROM(SELECT trm.id_rencana_header,id_rencana_detail,tanggal_rencana,trd.active,nama as veraa_user,name1,desa FROM trx_rencana_master trm 
								...
								JOIN trx_rencana_detail trd ON trm.id_rencana_header=trd.id_rencana_header JOIN mst_user mu ON mu.id_user=trm.id_user_input_rencana WHERE urgent='1' AND trm.id_rencana_header='$id_rencana_header')as data_union GROUP BY id_rencana_detail");
