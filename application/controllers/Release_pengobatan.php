...
class Release_pengobatan extends CI_Controller {

	public function index() {
		if($this->session->userdata('id_role') <=4 ) {
			$id_rph = $this->session->userdata("id_rph");
			$d['pengobatan'] = $this->App_model->get_pengobatan_sapi();
			$d['judul'] = 'Release Pengobatan';	
			// $d['pemotongan_sapi_ggl_stts'] = $this->App_model->get_penerimaan_detail_rph2_no_tgl_stts1($id_rph,'','2');	
			$this->load->view('top',$d);
			$this->load->view('menu');
			$this->load->view('release_pengobatan/release_pengobatan_table.php');
