...
class App_model extends CI_Model {
	...
	public function get_aproval_urgent($id="") {
		$q = $this->db->query("SELECT * FROM (SELECT COUNT(*)AS jml,trm.id_rencana_header,tanggal_penetapan,tanggal_rencana,aproved,veraa_user FROM trx_rencana_master trm 
							JOIN trx_rencana_detail trd ON trm.id_rencana_header=trd.id_rencana_header JOIN (SELECT lifnr,name1,desa,veraa_user 
							FROM trans_index GROUP BY lifnr)as petani ON trd.id_customer=petani.lifnr WHERE urgent='1' AND trm.active='1' GROUP BY trm.id_rencana_header
							union 
							...
							JOIN trx_rencana_detail trd ON trm.id_rencana_header=trd.id_rencana_header JOIN (SELECT lifnr,name1,desa,veraa_user 
							FROM trans_indexp GROUP BY lifnr)as petani ON trd.id_customer=petani.lifnr WHERE urgent='1' AND trm.active='1' GROUP BY trm.id_rencana_header)as data");
		return $q;
	}
