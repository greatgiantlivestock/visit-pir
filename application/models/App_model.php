...
class App_model extends CI_Model {
	...
	public function get_customer($id="") {
		...
		$q = $this->db->query("SELECT mst_customer.*,nama_wilayah,nama_status FROM mst_customer 
							...
							GROUP BY mst_customer.id_customer ORDER BY id_customer DESC");
	...
	*/
	...
	public function get_rekap_salesman($tanggal1="",$tanggal2="",$nama_karyawan="") {
		...
		if($this->session->userdata("id_role") <=2){
			...
			$id_departemen = $this->session->userdata("id_departemen");$q_rekap_biaya = $this->db->query("SELECT no_filter.*,nama_karyawan FROM (SELECT data_actual.*, tanggal_checkin, tanggal_checkout from (SELECT data_rencana.*,jml_visit_realisasi from 
	...
	public function getAllShipment($id="") {
		...
		// 						AS data_shipment WHERE wil_cust='$wilayah' AND id_status_kirim='1' GROUP BY cust_ship");
		...
		// }
			...
			if($q_user_shipping_point->jml > 0 ){
				// $q = $this->db->query("SELECT data_fix.* (SELECT nama,nama_wilayah,wil_cust,wil_user,cust_ship,description,date(tanggal_request) as tanggal_request,tanggal_kirim,catatan,id_shipping,
				...
				// 				AS data_shipment WHERE wil_cust='$wilayah' AND id_status_kirim='1' GROUP BY id_request,nama_transaksi)as data_fix
